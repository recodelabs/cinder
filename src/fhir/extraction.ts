...
// Use fhirpath's exported types where available, define locally otherwise.
type FPContext = Record<string, any>;
type FPModel = fhirpath.Model;
type FPOptions = { userInvocationTable?: fhirpath.UserInvocationTable };
...
function processTemplateString(
    ...
    node: string,
    context: FPContext,
    model: FPModel | undefined,
    fpOptions: FPOptions | undefined,
) {
    let match:
        | RegExpExecArray
        | null;
...
function processAssignBlock(
    ...
    if (assignKey) {
        if (Array.isArray(node[assignKey])) {
            node[assignKey].forEach((obj: any) => {
...
function processForBlock(
    ...
    if (forKey) {
        ...
        return {
            node: answers.map((answer: any, index: number) =>
                resolveTemplateRecur(
                    path,
                    resource,
                    node[forKey],
                    {
                        ...context,
                        [itemKey]: answer,
                        ...(hasIndexKey && indexKey ? { [indexKey]: index } : {}),
                    },
                    model,
                    fpOptions,
                ),
...
function processContextBlock(
    ...
    if (contextKey) {
        ...
        const answers = evaluateExpression(path, resource, expr, context, model, fpOptions);
        const result: any[] = answers.map((answer: any) =>
...
type Transformer = (path: Path, node: any, context: FPContext) => { node: any; context: FPContext };

function iterateObject(startPath: Path, obj: any, context: FPContext, transform: Transformer): any {

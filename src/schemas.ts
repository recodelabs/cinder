// ABOUTME: Loads FHIR R4 StructureDefinitions and SearchParameters on demand.
// ABOUTME: Uses dynamic imports so definitions are code-split into separate chunks.
...
export async function loadSchemas(): Promise<void> {
  ...
  const [profilesTypes, profilesResources, searchParameters, searchParametersMedplum, searchParametersUscore] =
    await Promise.all([
